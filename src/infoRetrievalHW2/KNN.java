...
import instanceBasedLearningAndFeatureSelection.TargetDistance;
...
public class KNN {
 ...
 static void separateData(int i, ArrayList<double[]> inputSample, ArrayList<double[]> expectedOutput,
			...
			ArrayList<double[]> testOutput) {//5 fold 
		switch (i)
		{
		case 0:
			for(int j = 0; j < inputSample.size()/5; j++)
			{
				testInput.add(inputSample.get(j));
				testOutput.add(expectedOutput.get(j));
			}
			for(int k = inputSample.size()/5; k < inputSample.size(); k++)
			{
		...
		case 1:
			...
			for(int k = inputSample.size()/5; k < inputSample.size() * 2/5; k++)
			{
				testInput.add(inputSample.get(k));
				testOutput.add(expectedOutput.get(k));
			}
			for(int l = inputSample.size() * 2/5; l < inputSample.size(); l++)
			{
			...
			break;
		case 2:
			for(int j = 0; j < inputSample.size() * 2/5; j++)
			{
				trainingInput.add(inputSample.get(j));
				trainingOutput.add(expectedOutput.get(j));
			}
			for(int k = inputSample.size() * 2/5; k < inputSample.size() * 3/5; k++)
			{
				testInput.add(inputSample.get(k));
				testOutput.add(expectedOutput.get(k));
			}
			for(int l = inputSample.size() * 3/5; l < inputSample.size(); l++)
			{
				trainingInput.add(inputSample.get(l));
				trainingOutput.add(expectedOutput.get(l));
			}
			break;
		case 3:
			for(int j = 0; j < inputSample.size() * 3/5; j++)
			{
				trainingInput.add(inputSample.get(j));
				trainingOutput.add(expectedOutput.get(j));
			}
			for(int k = inputSample.size() * 3/5; k < inputSample.size() * 4/5; k++)
			{
				testInput.add(inputSample.get(k));
				testOutput.add(expectedOutput.get(k));
			}
			for(int l = inputSample.size() * 4/5; l < inputSample.size(); l++)
			{
				trainingInput.add(inputSample.get(l));
				trainingOutput.add(expectedOutput.get(l));
			}
			break;
		case 4:
			for(int j = 0; j < inputSample.size() * 4/5; j++)
			{
				trainingInput.add(inputSample.get(j));
				trainingOutput.add(expectedOutput.get(j));
			}
			for(int k = inputSample.size() * 4/5; k < inputSample.size(); k++)
			{
				testInput.add(inputSample.get(k));
				testOutput.add(expectedOutput.get(k));
			}
 ...
 static double fiveFoldCrossValidation(int k, ArrayList<double[]> inputSample, 
		 ArrayList<double[]> expectedOutput)
	{//10-fold cross-validation
		double[] errRate = new double[10];
		for(int i = 0; i < 5; i++)
		{
			...
			int numOfYes = 0;
			int numOfNo = 0;
			int predictedYes = 0, predictedNo = 0;
			int preYesButNo = 0, preNoButYes = 0, preYesYes = 0, preNoNo = 0;
			for(int j = 0; j < testInput.size(); j++)
			{//for every test sample, calculate its distance between other training samples
				if(testInput.get(j)[testInput.get(j).length - 1] == 0)
				{//Yes!
					numOfYes++;
				}
				else//No
				...
				if(maxRank(sum) == 0 && maxRank(testOutput.get(j)) == 0) preYesYes++;
				else if(maxRank(sum) == 0 && maxRank(testOutput.get(j)) == 1) preYesButNo++;
				else if(maxRank(sum) == 1 && maxRank(testOutput.get(j)) == 0) preNoButYes++;
				else preNoNo++;
			...
			System.out.println("Predicted Yes and Actual Yes: " + preYesYes + " " + "Predicted Yes but Actual No: " + preYesButNo);
			System.out.println("Predicted No and Actual No: " + preNoNo + " " + "Predicted No but Actual Yes: " + preNoButYes);

			//System.out.println("Num of Yes: " + numOfYes + " " + "Num of No: " + numOfNo + " " + "Total test size: " 
			//+ testInput.size());
		...
		double sigma = Math.sqrt(squre/5);
		double se = sigma/(Math.sqrt(5)) * 100;
		System.out.print("5 Fold Cross Validation Error Rate: %");
		System.out.printf("%8.4f", mean*100);
		System.out.print("  \u00b1");
		System.out.printf("%8.4f", se);
		System.out.println("\t");
...
public static void main(String[] args) throws IOException
    ...
    Collections.shuffle(allTuples);//shuffle data
		...
		double oldErrRate = fiveFoldCrossValidation(k, inputSample, expectedOutput);
